...
public sealed partial class CompressGroup
{
	...
	{
		var userLock = CompressGroup.QueueLocks.GetOrAdd(Context.User.Id, _ => new SemaphoreSlim(1, 1));
	...
	private async Task DownloadVideo(string url, string? resolution)
	{
		...
		var runResult = await RunDownload(url, TimeSpan.FromMinutes(4),
			...
			new OptionSet
			{
				...
				NoPlaylist = true,
				RemuxVideo = CompressGroup.RemuxVideo
			}, Context.Interaction);
	...
	private async Task CompressVideo(string before, string after, string? resolution, bool resolutionChange)
	{
		...
		if (resolutionChange)
		{
			var resolutionChoices = typeof(DougaBot.Modules.CompressGroup.CompressGroup)
